    ...
    <div className="container mx-auto p-4 relative">
      <div className="flex flex-col md:flex-row  bg-gray-600 shadow-lg text-white rounded-lg overflow-hidden">
        ...
        <div className="md:w-2/3 p-4 flex flex-col gap-5">
          <h2 className="text-2xl font-semibold">{data ? data.original_title : ""}</h2>
          ...
          <h2 className="font-semibold pt-2">Release-date : {data.release_date }</h2>
             <div className="flex gap-3 pt-3">
         {  data?.genres?.map((item) => (
              <h5 key={item.id} className="text-2xl font-semibold mr-2 rounded border border-[blue] p-2">
                {item.name}
              </h5>
            ))
          }
     ...
     <div className='absolute top-5 left-6' onClick={handleadd} >
      ...
      {
      toggle ?
      <img className='cursor-pointer ' src={addimg}/>: <span className="text-[30px] cursor-pointer ">✅</span>
    }
